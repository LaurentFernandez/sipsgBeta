<div class="card">
        ...
        <div class="d-flex">
            ...
            <div class="ms-auto text-secondary">
                ...
                <div class="ms-2 d-inline-block">
                    <input type="text" wire:model.live="search" class="form-control form-control-sm" aria-label="Buscar cotización">
                </div>
            </div>
        </div>
    </div>
                ...
                <tr>
                    ...
                    <td class="align-middle text-center">
                        ...
                        @if ($quotation->status === \App\Enums\QuotationStatus::PENDING)
                            <x-button.complete class="btn-icon" route="{{ route('quotations.update', $quotation->uuid) }}" onclick="return confirm('¿Está seguro de completar la cotización núm. {{ $quotation->reference }}?')"/>
                            <x-button.delete class="btn-icon" route="{{ route('quotations.destroy', $quotation) }}" onclick="return confirm('¿Está seguro de cancelar la Cotización NO. {{ $quotation->reference }}?')"/>
                        @endif
                    </td>
                </tr>
            @empty
    ...
    <div class="card-footer d-flex align-items-center">
        <p class="m-0 text-secondary">
            Mostrando <span>{{ $quotations->firstItem() }}</span> a <span>{{ $quotations->lastItem() }}</span> de <span>{{ $quotations->total() }}</span> entradas
        </p>
